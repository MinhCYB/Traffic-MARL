...
\begin{equation}
    ...
    \label{eq:gat_attn}
\end{equation}
trong đó $\text{LReLU}(\cdot)$ là LeakyReLU. Với multi-head attention ($K$ heads), output được average (tương ứng với \texttt{concat=False} trong triển khai):
\begin{equation}
...
\end{equation}
Cách average này (thay vì concatenate) giữ nguyên chiều output $d_h = 64$ bất kể số heads $K$, cho phép Q-head nhận input có kích thước cố định.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=\linewidth]{imgs/gat_marl_architecture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{imgs/learning_curves_mydinh.png}
    \caption{Learning curves so sánh GAT-MARL, IDQN, và Fixed-time trên map Mỹ Đình (500 episodes, parallel training 2 workers). Đường liền là smoothed mean (rolling window 20 eps), vùng bóng là $\pm 1$ std. Điểm số cuối (50 eps cuối) được annotate tại phía phải panel (a).}
...
\textit{Lưu ý về throughput}: Fixed-time đạt throughput cao hơn hai phương pháp RL vì chu kỳ đèn cố định, đều đặn cho phép SUMO dispatch xe upstream liên tục vào mạng. Các phương pháp RL đôi khi giữ phase lâu hơn để giải tỏa queue --- giảm waiting time nhưng tạm thời giảm capacity tiếp nhận xe mới. Điều này phản ánh đặc thù của mô phỏng vi mô (SUMO): throughput phụ thuộc vào cả lượng xe được inject vào hệ thống, không chỉ số xe đã rời mạng.

\subsection{Khả năng thích ứng sự cố}

\begin{table}[h]
\centering
\caption{Hiệu năng khi có vật cản (obstacle\_prob = 0.3)}
\label{tab:obstacle}
\setlength{\tabcolsep}{4pt}
...
\begin{table}[!t]
\centering
\caption{So sánh hiệu năng và tốc độ học trên map UET (15 ngã tư, 50 eps cuối)}
\label{tab:transfer}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{2.0cm} c c p{1.8cm}}
\toprule
\textbf{Phương pháp} & \textbf{Eps$^{*}$} & \textbf{Reward} & \textbf{Chiến lược} \\
\midrule
GAT-MARL (finetune) & \textbf{3}  & $-560.4 \pm 90.4$ & Freeze/unfreeze 2-phase \\
IDQN (scratch)      & 21          & $-676.9 \pm 309.0$ & Train lại từ đầu \\
Fixed-time          & ---         & $-750.8 \pm 292.8$ & Không học \\
\bottomrule
\end{tabular}
\smallskip\\
\footnotesize{$^{*}$Eps: số episodes để đạt mức hiệu năng ổn định (rolling-5 mean $\geq -569$).}
\end{table}

GAT-MARL tận dụng checkpoint từ Mỹ Đình qua cơ chế \textbf{2-phase fine-tuning} (Section~\ref{sec:finetune}): GAT layer giữ lại \textit{graph topology knowledge} đã học --- cách aggregate thông tin từ neighbors --- và chỉ cần re-calibrate Q-head cho distribution mới. Nhờ đó, chỉ sau \textbf{3 episodes} fine-tuning, GAT-MARL đã đạt mức hiệu năng ổn định tương đương kết quả cuối cùng của chính nó, trong khi IDQN cần \textbf{21 episodes} train from scratch để đạt cùng ngưỡng đó. Ngoài ra, kết quả cuối cùng của GAT-MARL (Reward $-560.4 \pm 90.4$) vượt trội hơn IDQN ($-676.9 \pm 309.0$) cả về giá trị trung bình lẫn độ ổn định --- std của GAT-MARL thấp hơn $3.4\times$, xác nhận rằng knowledge transfer qua GAT không chỉ giúp \textit{học nhanh hơn} mà còn \textit{ổn định hơn} trên topology phức tạp hơn. IDQN không có communication layer tách biệt nên phải học lại toàn bộ từ đầu khi chuyển sang map hoặc scenario mới.
...
\textbf{Độ ổn định trên map lớn (UET)}: một điểm cộng đáng chú ý của GAT-MARL khi triển khai trên topology phức tạp hơn là độ ổn định vượt trội so với IDQN. Trên map UET (50 episodes cuối), std của GAT-MARL là $90.4$ so với $309.0$ của IDQN --- thấp hơn $3.4\times$. Điều này có ý nghĩa thực tế: reward của GAT-MARL ít biến động giữa các episodes, tức là hiệu năng đèn giao thông ổn định hơn theo thời gian, không bị giảm đột ngột khi gặp kịch bản bất thường. Cơ chế GAT giúp mỗi agent ``nhìn thấy'' trạng thái neighbors, từ đó phản ứng phối hợp thay vì phản ứng đơn lẻ khi lưu lượng thay đổi --- đây là lợi thế cấu trúc mà IDQN không có.
...
Thực nghiệm trên map Mỹ Đình (50 episodes cuối) cho thấy cả hai phương pháp RL đều vượt trội so với Fixed-time về Global Reward ($-260$ và $-340$ so với $-364$) và Avg Wait ($3.99$s và $4.46$s so với $5.18$s). Thực nghiệm chuyển giao từ Mỹ Đình sang UET cho thấy GAT-MARL đạt mức hiệu năng ổn định chỉ sau \textbf{3 episodes} fine-tuning, trong khi IDQN cần \textbf{21 episodes} train from scratch để đạt cùng ngưỡng đó. Hơn nữa, kết quả cuối cùng trên UET của GAT-MARL ($-560.4 \pm 90.4$) ổn định hơn IDQN ($-676.9 \pm 309.0$) với độ lệch chuẩn thấp hơn $3.4\times$, xác nhận giá trị của việc tách biệt communication layer trong thiết kế kiến trúc MARL.
