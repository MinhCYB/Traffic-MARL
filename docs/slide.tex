...
\begin{frame}{Kiến trúc mô hình GAT-MARL}
  \begin{columns}
    ...
    \column{0.55\textwidth}
      % [GỢI Ý ẢNH] Sơ đồ kiến trúc — dùng ảnh có sẵn:
...
\begin{frame}{Graph Attention Network — Cơ chế giao tiếp}
  \begin{columns}
    ...
    \column{0.5\textwidth}
      % [GỢI Ý ẢNH] Attention heatmap 8×8 giữa các ngã tư Mỹ Đình —
      % thể hiện agent nào đang "chú ý" neighbor nào tại giờ cao điểm:
...
\begin{frame}{Parallel Training — Ape-X Style}
  \begin{columns}
    ...
    \column{0.55\textwidth}
      % [GỢI Ý ẢNH] Sơ đồ Ape-X Workers → Queue → Learner:
    ...
    \column{0.5\textwidth}
      ...
      \begin{center}
        \begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
          \draw[fill=primary!20, rounded corners] (0,2) rectangle (3,3)
            node[midway] {Encoder};
          \draw[fill=accent!30, rounded corners] (0,1) rectangle (3,2)
            node[midway] {\textbf{GAT} (frozen)};
          \draw[fill=primary!20, rounded corners] (0,0) rectangle (3,1)
            node[midway] {Q-head (update)};
          \node[right] at (3.1, 1.5) {\footnotesize Phase 1};
          \draw[->, thick, accent] (3.6,1.5) -- (4.4,1.5);
          \draw[fill=primary!20, rounded corners] (4.5,2) rectangle (7.5,3)
            node[midway] {Encoder};
          \draw[fill=primary!40, rounded corners] (4.5,1) rectangle (7.5,2)
            node[midway] {\textbf{GAT} (unfreeze)};
          \draw[fill=primary!20, rounded corners] (4.5,0) rectangle (7.5,1)
            node[midway] {Q-head};
          \node[right] at (7.6, 1.5) {\footnotesize Phase 2};
        \end{tikzpicture}
      \end{center}
  \end{columns}
\end{frame}
...
\begin{frame}{Kết quả — Map Mỹ Đình (500 episodes)}
  \begin{columns}
    \column{0.55\textwidth}
      % [GỢI Ý ẢNH] Learning curves 3 phương pháp:
...
\begin{frame}{Transfer Learning — Mỹ Đình $\to$ UET}
  \begin{columns}
    ...
    \column{0.5\textwidth}
      % [GỢI Ý ẢNH] Learning curve so sánh GAT finetune vs IDQN scratch
      % trên map UET (nếu có ảnh riêng). Nếu không, dùng ảnh topology UET:
...
\begin{frame}{Dashboard Real-time}
  ...
  \begin{center}
    \includegraphics[width=0.9\linewidth]{imgs/live_demo_mydinh.png}
  \end{center}
  \vspace{0.2em}
...
\begin{frame}{Kết luận}
  \begin{block}{Đóng góp chính}
    \begin{itemize}
      ...
      \item \textbf{2-phase fine-tuning}: GAT-MARL hội tụ sau 20 ep thay vì 25 ep của IDQN khi chuyển topology
