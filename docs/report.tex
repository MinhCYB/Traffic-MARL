...
Báo cáo này trình bày hệ thống GAT-MARL cho mạng lưới giao thông thực tế khu vực Mỹ Đình (8 ngã tư), với các nội dung chính:
...
Điểm khác biệt quan trọng so với Ape-X gốc~\cite{ApeX2018}: epsilon của mỗi worker là \textbf{cố định suốt quá trình training} (\texttt{epsilon\_decay=1.0}, \texttt{epsilon\_min=epsilon\_start}), không decay theo episode. Trong Ape-X gốc, các worker vẫn sử dụng epsilon decay (chỉ khác nhau về tốc độ decay), dẫn đến tất cả worker dần hội tụ về cùng mức epsilon thấp.
...
Đây là \textbf{lợi thế cạnh tranh} so với IDQN: vì IDQN không có communication layer tách biệt, toàn bộ network là một khối MLP đồng nhất --- không có module nào chứa ``transferable knowledge'' riêng. Khi đổi map hoặc scenario, IDQN phải train lại từ đầu hoặc chấp nhận negative transfer.
...
\textbf{Kết luận}: normalize đảm bảo $\alpha, \beta$ \textit{thực sự} là tỉ lệ đóng góp giữa hai thành phần reward, không chỉ là hệ số nhân vô nghĩa trên hai đại lượng khác đơn vị.
...
Báo cáo trình bày toàn bộ quá trình xây dựng hệ thống điều khiển đèn giao thông thông minh GAT-MARL, kết hợp Graph Attention Network với Multi-Agent Deep Q-Learning trên mạng lưới giao thông thực tế Mỹ Đình (8 ngã tư). Các điểm nổi bật kỹ thuật bao gồm: reward function hybrid với normalization, parallel training với fixed-role epsilon workers, dynamic lane detection, và obstacle injection cho robustness.
